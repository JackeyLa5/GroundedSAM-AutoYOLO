...
| `use_grounded_sam_seg` | bool | `true` | 启用 Grounded-SAM mask 提取 |
| `grounded_sam_threshold` | float | `0.5` | Grounded-SAM 检测置信度阈值（0–1） |
| `grounded_sam_mask_threshold` | float | `0.5` | Grounded-SAM mask 二值化阈值（0–1） |
| `grounded_sam_text` | string | `""` | 覆盖 Grounded-SAM 文本提示（默认使用分类名拼接） |

## 检测与标注

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/detect` | Grounded-SAM 检测与可选实例分割（multipart） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/model/events` | SSE 流：Grounded-SAM 服务状态推送 |
| GET | `/model/grounded-sam/status` | Grounded-SAM 状态（`loaded`、`status`：`starting`/`loading`/`loaded`/`unloaded`） |
| POST | `/model/grounded-sam/unload` | 停止 Grounded-SAM 服务进程 |
...
新记录的 `modelType` 为 `"grounded-sam"`；已有持久化记录可能保留历史值。
